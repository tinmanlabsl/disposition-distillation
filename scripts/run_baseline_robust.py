#!/usr/bin/env python3
"""
DD v2 Baselines — Robust version with incremental saves and resume.
Qwen3.5-0.8B HumanEval + LoRA, Gemma 4 E2B LoRA.
"""
import json, time, re, torch, traceback, subprocess, gc, os, signal, sys

CACHE = "/workspace/persistent/eval/humaneval_problems.json"
RESULTS_DIR = "/workspace/persistent/eval"
CHECKPOINT = f"{RESULTS_DIR}/baseline_checkpoint.json"
...
def run_qwen_humaneval():
    ...
    results = ckpt.get("qwen35_humaneval_results", [])
    done_ids = {r["task_id"] for r in results}
    passed = sum(1 for r in results if r["passed"])
    if results:
        print(f"  Resuming from checkpoint: {len(results)}/{len(problems)} done, {passed} passed", flush=True)

    start = time.time()

    for i, p in enumerate(problems):
        if p["task_id"] in done_ids:
            continue
        ...
        if len(results) % 10 == 0:
            save_checkpoint({"qwen35_humaneval_results": results})
        ...
        if len(results) % 20 == 0:
            ...
            print(f"  [{len(results)}/164] {passed}/{len(results)} ({passed/len(results)*100:.1f}%) | {rate:.0f} prob/min", flush=True)
