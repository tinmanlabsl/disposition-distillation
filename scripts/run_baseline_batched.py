#!/usr/bin/env python3
"""
DD v2 Baselines — Batched parallel generation for speed.
Qwen3.5-0.8B HumanEval + LoRA, Gemma 4 E2B LoRA.
"""
import json, time, re, torch, traceback, subprocess, gc, os, signal

CACHE = "/workspace/persistent/eval/humaneval_problems.json"
RESULTS_DIR = "/workspace/persistent/eval"
BATCH_SIZE = 8  # Process 8 problems at once
...
# ── Part A: Qwen3.5 HumanEval (batched) ─────────────────────────────────────

def run_qwen_humaneval():
    print("\n" + "=" * 60)
    print(f"PART A: Qwen3.5-0.8B HumanEval (164 problems, batch={BATCH_SIZE})")
    print("=" * 60, flush=True)
    ...
    for batch_start in range(0, len(problems), BATCH_SIZE):
        ...
        batch_texts = all_input_texts[batch_start:batch_end]
        batch_problems = problems[batch_start:batch_end]

        # Tokenize batch with left-padding
        batch_enc = tok(batch_texts, return_tensors="pt", padding=True, truncation=True, max_length=2048)
        batch_enc = {k: v.to(model.device) for k, v in batch_enc.items()}

        with torch.no_grad():
            outputs = model.generate(
                **batch_enc, max_new_tokens=1024, do_sample=False,
                pad_token_id=tok.eos_token_id,
            )

        # Decode each response
        for j, (prob, input_text) in enumerate(zip(batch_problems, batch_texts)):
            input_len = batch_enc["attention_mask"][j].sum().item()
            resp = tok.decode(outputs[j][input_len:], skip_special_tokens=True)
            comp = extract_code(resp)
            ok = check_solution(prob, comp)
            if ok:
                passed += 1

            results.append({
                "task_id": prob["task_id"], "passed": ok,
        ...
        if done == BATCH_SIZE:
            gpu_check("after_first_batch")

        if done % 20 == 0 or done == len(problems):
            el = time.time() - start
            rate = done / el * 60 if el > 0 else 0
            print(f"  [{done}/164] {passed}/{done} ({passed/done*100:.1f}%) | {rate:.0f} prob/min", flush=True)

        # Save checkpoint every 40 problems
        if done % 40 == 0:
            save_json({"partial_results": results, "passed": passed}, f"{RESULTS_DIR}/qwen35_checkpoint.json")
    ...
    print(f"  Saved: {RESULTS_DIR}/qwen35_humaneval_final.json", flush=True)
...
def run_lora_sanity(model_name, model_path):
    ...
    print(f"  Saved: {RESULTS_DIR}/{safe_name}_lora_final.json", flush=True)
