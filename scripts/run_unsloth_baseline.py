#!/usr/bin/env python3
"""
DD v2 Baselines via Unsloth — Qwen3.5 HumanEval + LoRA, Gemma 4 E2B LoRA
Runs on RTX 4090 with Unsloth for proper GPU execution.
"""
...
def run_qwen_humaneval():
    ...
    model, tok = FastLanguageModel.from_pretrained(
        model_name="/workspace/persistent/models/qwen3.5-0.8b",
        max_seq_length=2048,
        dtype=None,
        load_in_4bit=False,
    ...
    for i, p in enumerate(problems):
        ...
        if (i + 1) % 20 == 0:
            el = time.time() - start
            print(
                f"  [{i+1}/164] {passed}/{i+1} ({passed/(i+1)*100:.1f}%) "
                f"| {(i+1)/el*60:.0f} prob/min",
                flush=True,
            )
    ...
    print(f"\n  RESULT: Pass@1 = {passed}/{len(problems)} = {score:.1f}%", flush=True)
    print(f"  Think blocks: {think_ct}, Time: {el:.0f}s", flush=True)
    ...
    result = {
        ...
        "passed": passed,
        "total": len(problems),
        "think_blocks": think_ct,
        "elapsed": el,
        "results": results,
    }
    save_json(result, f"{RESULTS_DIR}/qwen35_humaneval_4090.json")
...
def run_lora_sanity(model_name, model_path):
    ...
    print(
        f"\n  LoRA VERDICT: {model_name} — "
        f"think={think_blocks}/10, garbage={garbage}/10 → {'PASS' if verdict else 'FAIL'}",
        flush=True,
    )
    ...
    save_json(result, f"{RESULTS_DIR}/{model_name.replace('-', '_').replace('.', '')}_lora_4090.json")
...
if __name__ == "__main__":
    ...
    # Part A: Qwen3.5 HumanEval
    try:
        all_results["qwen35_humaneval"] = run_qwen_humaneval()
    except Exception as e:
        print(f"\nERROR Part A (Qwen3.5 HumanEval): {e}")
        traceback.print_exc()
